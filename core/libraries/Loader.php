...
class SP_Loader {
	...
	var $_ci_is_instance 	= FALSE; // Whether we should use $this or $SP =& get_instance()
	...
	{		
		...
		if (isset($SP->$name))
		{
			ssp_show_error('The model name you are loading is the name of a resource that is already being used: '.$name);
		}
		...
		{
			ssp_show_error('Unable to locate the model you have specified: '.$model);
		}
				
		if ($db_conn !== FALSE AND ! class_exists('SP_DB'))
		{
			if ($db_conn === TRUE)
				$db_conn = '';
		
			$SP->load->database($db_conn, FALSE, TRUE);
		}
	...
	function database($params = '', $return = FALSE, $active_record = FALSE)
	{
		...
		if (class_exists('SP_DB') AND $return == FALSE AND $active_record == FALSE AND isset($SP->db) AND is_object($SP->db))
		{
			return FALSE;
		}	
	...
	function dbutil()
	{
		...
		require_once(BASEPATH.'database/drivers/'.$SP->db->dbdriver.'/'.$SP->db->dbdriver.'_utility'.EXT);
		$class = 'SP_DB_'.$SP->db->dbdriver.'_utility';

		$SP->dbutil =& instantiate_class(new $class());

		$SP->load->_ci_assign_to_models();
	}
	...
	function dbforge()
	{
		...
		require_once(BASEPATH.'database/drivers/'.$SP->db->dbdriver.'/'.$SP->db->dbdriver.'_forge'.EXT);
		$class = 'SP_DB_'.$SP->db->dbdriver.'_forge';

		$SP->dbforge = new $class();
	...
	function helper($helpers = array())
	{
		...
		{		
			...
			if (file_exists($ext_helper))
			{
				...
				{
					ssp_show_error('Unable to load the requested file: helpers/'.$helper.EXT);
				}
	...
	function plugin($plugins = array())
	{
		...
		{	
			...
			else
			{
				...
				{
					ssp_show_error('Unable to load the requested file: plugins/'.$plugin.EXT);
				}
			}
	...
	{
		$SP =& get_instance();
		return $SP->lang->load($file, $lang, $return);
	}
	...
	{			
		$SP =& get_instance();
		$SP->config->load($file, $use_sections, $fail_gracefully);
	}
	...
	{		
		if ($table === FALSE)
		{
			ssp_show_error('You must include the name of the table you would like to access when you initialize scaffolding');
		}
		
		$SP =& get_instance();
		$SP->_ci_scaffolding = TRUE;
		$SP->_ci_scaff_table = $table;
	}
	...
	function _ci_load($_ci_data)
	{
		...
		{
			ssp_show_error('Unable to load the requested file: '.$_ci_file);
		}
		...
		{
			$_ci_SP =& get_instance();
			foreach (get_object_vars($_ci_SP) as $_ci_key => $_ci_var)
			{
				if ( ! isset($this->$_ci_key))
				{
					$this->$_ci_key =& $_ci_SP->$_ci_key;
				}
			}
		}
		...
		foreach (array(ucfirst($class), strtolower($class)) as $class)
		{
			...
			if (file_exists($subclass))
			{
				...
				if ( ! file_exists($baseclass))
				{
					...
					ssp_show_error("Unable to load the requested class: ".$class);
				}
	...
	{	
		...
		if ($prefix == '')
		{			
			if (class_exists('SP_'.$class)) 
			{
				$name = 'SP_'.$class;
				log_message('debug', 'Loading '.$class. ' Class');
			}
		...
		if ( ! class_exists($name))
		{
			...
			ssp_show_error("Non-existent class: ".$class);
		}
		...
		if ($config !== NULL)
		{
			$SP->$classvar = new $name($config);
		}
		else
		{		
			$SP->$classvar = new $name;
		}	
	} 	
	...
	function _ci_is_instance()
	{
		...
		global $SP;
		return (is_object($SP)) ? TRUE : FALSE;
	}
