 ...
 */
class Ssp_javascript {

	var $_javascript_location = 'js';

	function Ssp_javascript($params = array())
	{
		$defaults = array('js_library_driver' => 'ssp_jquery', 'autoload' => TRUE);
		...
		$this->SP->load->library('js/'.$js_library_driver, array('autoload' => $autoload));
	...
	function external($external_file = '', $relative = FALSE)
	{
		...
		{
			if ($this->SP->config->item('javascript_location') != '')
			{
	...
	{
		$str = '<script type="text/javascript" charset="'.strtolower($this->SP->config->item('charset')).'"';
		$str .= ($src == '') ? '>' : ' src="'.$src.'">';
		return $str;
	}
